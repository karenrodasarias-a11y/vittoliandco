...
function StatusBadge({ status }) {
  const s = statusConfig[status] || { label: status, bg: C.beige, color: C.brownMid };
    ...
    <motion.div whileHover={{ y: -8 }} onHoverStart={() => setHover(true)} onHoverEnd={() => setHover(false)}
      ...
      <div style={{ position: "relative", aspectRatio: "1/1.1", background: product.bg || C.roseLight, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
        ...
        <motion.button animate={{ opacity: hover ? 1 : 0, scale: hover ? 1 : 0.8 }} onClick={e => { e.stopPropagation(); onWishlist(product.id); }}
          style={{ position: "absolute", top: 12, right: 12, width: 36, height: 36, borderRadius: "50%", background: "white", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 12px rgba(0,0,0,0.1)", color: inWish ? C.roseDeep : C.brownMid }}>
          ...
          <>
            ...
            <div style={{ marginTop: 24, paddingTop: 20, borderTop: `1px solid ${C.beige}` }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20, fontWeight: 700, fontSize: 18 }}>
                <span style={{ color: C.brownDark }}>Total</span>
    ...
    <Modal open onClose={onClose} title="Finalizar compra" width={640}>
        ...
        {steps.map((s, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", flex: i < steps.length - 1 ? 1 : 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
              <div style={{ width: 30, height: 30, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, background: i <= step ? C.roseDeep : C.beige, color: i <= step ? "white" : C.brownMid }}>
                {i < step ? <Icon d={Icons.check} size={14} /> : i + 1}
              </div>
              <span style={{ fontSize: 13, fontWeight: 600, color: i === step ? C.brownDark : C.brownMid }}>{s}</span>
            </div>
        ...
        <div>
          <div style={{ maxHeight: 280, overflowY: "auto", marginBottom: 20 }}>
            {cart.map(item => (
              <div key={item.id} style={{ display: "flex", gap: 14, padding: "12px 0", borderBottom: `1px solid ${C.beige}` }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: 14, color: C.brownDark }}>{item.name}</div>
                  <div style={{ fontSize: 13, color: C.brownMid }}>x{item.qty} · S/. {item.price.toFixed(2)} c/u</div>
                </div>
          ...
          <div style={{ borderTop: `1px solid ${C.beige}`, paddingTop: 16 }}>
            {[["Subtotal", `S/. ${subtotal.toFixed(2)}`], discount > 0 && ["Descuento", `-S/. ${discount.toFixed(2)}`], ["Envío", shipping === 0 ? "¡GRATIS!" : `S/. ${shipping.toFixed(2)}`]].filter(Boolean).map(([k, v]) => (
              <div key={k} style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, fontSize: 14, color: C.brownMid }}><span>{k}</span><span style={{ color: k === "Descuento" ? C.success : C.brownDark }}>{v}</span></div>
            ))}
      ...
      {step === 1 && (
        <div>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <button onClick={() => setStep(0)} style={{ flex: 1, padding: "13px", borderRadius: 100, border: `1.5px solid ${C.beigeDark}`, background: "transparent", cursor: "pointer", fontWeight: 600, color: C.brownMid }}>← Volver</button>
      ...
      {step === 2 && (
        <div>
          ...
          {(payMethod === "stripe" && config.stripeEnabled) && (
            <div style={{ background: C.beige, borderRadius: 16, padding: 16, marginBottom: 20, fontSize: 13, color: C.brownMid }}>
          ...
          <div style={{ background: C.beige, borderRadius: 16, padding: 16, marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700, fontSize: 17 }}>
              <span style={{ color: C.brownDark }}>Total a pagar:</span>
...
function Storefront({ products, categories, config, coupons, cart, setCart, wishlist, setWishlist, orders, setOrders }) {
  ...
  return (
    <div>
      ...
      {/* Benefits */}
      <section style={{ padding: "80px", background: C.brownDark }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 24 }}>
          {[["🚀", "Envíos rápidos", "24-48 horas a todo el Perú"], ["🔒", "Pago 100% seguro", "Múltiples métodos de pago"], ["🌿", "Calidad premium", "Materiales certificados y seguros"], ["💬", "Atención personalizada", "WhatsApp 7 días a la semana"], ["↩️", "Cambios fáciles", "Sin costo adicional en 15 días"]].map(([icon, title, desc]) => (
            <div key={title} style={{ textAlign: "center", padding: "28px 16px", borderRadius: 20, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
              ...
              <div style={{ fontFamily: "serif", fontSize: 16, fontWeight: 600, color: C.cream, marginBottom: 8 }}>{title}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1.5fr", gap: 40, marginBottom: 40 }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {["📸", "📘", "🎵", "📌"].map(icon => (
                <a key={icon} href="#" style={{ width: 38, height: 38, borderRadius: "50%", background: "rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none", fontSize: 16, color: C.cream }}>{icon}</a>
              ))}
            </div>
          </div>
          <div>
            <h4 style={{ fontFamily: "serif", fontSize: 16, fontWeight: 600, color: C.cream, marginBottom: 16 }}>Tienda</h4>
          ...
          <div>
            <h4 style={{ fontFamily: "serif", fontSize: 16, fontWeight: 600, color: C.cream, marginBottom: 16 }}>Ayuda</h4>
          ...
          <div>
            <h4 style={{ fontFamily: "serif", fontSize: 16, fontWeight: 600, color: C.cream, marginBottom: 16 }}>Contacto</h4>
            ...
            <tbody>
              {orders.slice(0, 6).map(o => (
                <tr key={o.id} style={{ borderBottom: `1px solid ${C.beige}` }}>
                  <td style={{ padding: "12px", fontSize: 13, fontWeight: 700, color: C.brownDark }}>{o.orderNumber}</td>
                  <td style={{ padding: "12px", fontSize: 13, color: C.brownMid }}>{o.customerName}</td>
  ...
  const FormModal = (
    <Modal open={!!modal} onClose={() => setModal(null)} title={modal === "new" ? "Nuevo producto" : "Editar producto"} width={680}>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
        <button onClick={() => setModal(null)} style={{ flex: 1, padding: "13px", borderRadius: 100, border: `1.5px solid ${C.beigeDark}`, background: "transparent", cursor: "pointer", fontWeight: 600, color: C.brownMid }}>Cancelar</button>
              ...
              return (
                <tr key={p.id} style={{ borderTop: `1px solid ${C.beige}`, opacity: p.active ? 1 : 0.5 }}>
                  <td style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div><div style={{ fontWeight: 600, fontSize: 14, color: C.brownDark }}>{p.name}</div>{p.featured && <span style={{ fontSize: 10, color: C.sand, fontWeight: 700 }}>⭐ Destacado</span>}</div>
                  </td>
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={() => openEdit(p)} style={{ width: 34, height: 34, borderRadius: 10, border: `1.5px solid ${C.beigeDark}`, background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: C.brownMid }}>
          ...
          <tbody>
            {filtered.map(o => (
              <tr key={o.id} style={{ borderTop: `1px solid ${C.beige}` }}>
                <td style={{ padding: "14px 16px", fontWeight: 700, fontSize: 14, color: C.brownDark }}>{o.orderNumber}</td>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 20 }}>
        {coupons.map(c => (
          <div key={c.id} style={{ background: C.white, borderRadius: 20, padding: 24, boxShadow: "0 4px 24px rgba(139,110,82,0.08)", borderLeft: `4px solid ${c.active ? C.roseDeep : C.beigeDark}` }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => { setForm({ ...c, value: String(c.value), minAmount: c.minAmount ? String(c.minAmount) : "", maxUses: c.maxUses ? String(c.maxUses) : "" }); setModal(c); }} style={{ flex: 1, padding: "9px", borderRadius: 100, border: `1.5px solid ${C.beigeDark}`, background: "transparent", cursor: "pointer", fontSize: 13, fontWeight: 600, color: C.brownMid }}>
                ✏️ Editar
              </button>
              <button onClick={() => { navigator.clipboard?.writeText(c.code); toast("📋 Código copiado: " + c.code); }} style={{ flex: 1, padding: "9px", borderRadius: 100, border: `1.5px solid ${C.beigeDark}`, background: "transparent", cursor: "pointer", fontSize: 13, fontWeight: 600, color: C.brownMid }}>
                📋 Copiar
              </button>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            ...
            <div style={{ gridColumn: "1/-1" }}>
              <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 600, color: C.brownMid }}>Mostrar banner promocional en la tienda</span>
              </label>
            </div>
          </div>
        )}
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                ...
                <button onClick={() => { setReviews(rv => rv.map(x => x.id === r.id ? { ...x, approved: !x.approved } : x)); toast(r.approved ? "Reseña ocultada" : "✅ Reseña aprobada"); }}
                  style={{ padding: "4px 14px", borderRadius: 100, border: `1.5px solid ${C.beigeDark}`, background: "transparent", cursor: "pointer", fontSize: 11, fontWeight: 600, color: C.brownMid }}>
...
function AdminLogin({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(135deg, ${C.roseLight} 0%, ${C.skyLight} 100%)`, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} style={{ background: C.white, borderRadius: 28, padding: 48, width: "100%", maxWidth: 440, boxShadow: "0 32px 80px rgba(139,110,82,0.15)" }}>
        ...
        <form onSubmit={submit}>
          ...
          <div style={{ background: C.beige, borderRadius: 12, padding: "10px 16px", marginBottom: 20, fontSize: 12, color: C.brownMid }}>
  ...
  if (!loaded) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.cream }}>
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ fontFamily: "serif", fontSize: 20, color: C.brownMid }}>Cargando Venetus Kids...</p>
      </div>
    </div>
  );
